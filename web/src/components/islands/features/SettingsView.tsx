...
export const SettingsView: React.FC<SettingsViewProps> = ({ projectId }) => {
  ...
  // Local state for permissions changes (to enable/disable the Save button)
  ...
  useEffect(() => {
    if (detail && detail.projectId === projectId) {
      setLocalPerms({ ...detail.aiPermissions });
      setIsChanged(false);
    }
  }, [detail, projectId]);

  if (!detail || detail.projectId !== projectId || !localPerms) {
    return (
  ...
  const handleToggle = (key: keyof typeof localPerms) => {
    // Disabled rule: Access external tools requires at least one MCP server
    if (key === 'accessExternalTools' && detail.mcpServers.length === 0) {
    ...
    setIsChanged(JSON.stringify(nextPerms) !== JSON.stringify(detail.aiPermissions));
  };
  ...
  const handleAddMcp = (e: React.FormEvent) => {
    ...
    addMCPServer(mcpName.trim(), mcpUrl.trim());
    setIsMcpOpen(false);
    setMcpName('');
    setMcpUrl('');
  };
    ...
    <div className="max-w-[720px] mx-auto px-8 py-12 w-full fade-up flex flex-col gap-8 select-none">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col border border-border-subtle bg-surface/30 rounded-sm divide-y divide-border-subtle">
          ...
          <div
            className={`flex justify-between items-center p-5 ${detail.mcpServers.length === 0 ? 'opacity-38 cursor-not-allowed' : ''
              }`}
          >
            <div>
            ...
            <button
              disabled={detail.mcpServers.length === 0}
              onClick={() => handleToggle('accessExternalTools')}
              className={`relative w-10 h-[22px] rounded-pill p-0.5 transition-colors duration-200 focus:outline-none ${localPerms.accessExternalTools && detail.mcpServers.length > 0 ? 'bg-primary' : 'bg-border'
                }`}
            >
              <div
                className={`w-[18px] h-[18px] rounded-full transition-transform duration-200 ${localPerms.accessExternalTools && detail.mcpServers.length > 0
                  ? 'translate-x-[18px] bg-text-inverse'
                  : 'translate-x-0 bg-text-muted'
                  }`}
              />
                      ...
                      >
                        {['VIEWER', 'EDITOR', 'APPROVER'].map((r) => (
                          <button
                            key={r}
                            onClick={() => {
                              updateTeammateRole(member.id, r as any);
