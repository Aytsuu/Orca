...
- [ ] Initialize frontend repo (Astro + React + TypeScript + Tailwind)
- [ ] Initialize backend repo (FastAPI, Python 3.11+)
- [ ] Create Supabase project — get URL, anon key, service key
- [ ] Create `.env` files for both frontend and backend
- [ ] Confirm both dev servers run locally

---

## Phase 1 — Database Schema

- [ ] `project` table — id, name, description, created_at
- [ ] `project_member` table — id, project_id, session_id, role (`creator` | `approver` | `member`), can_approve, can_edit
- [ ] `chat_message` table — id, project_id, session_id, content, created_at
- [ ] `uploaded_file` table — id, project_id, session_id, storage_path, mime_type, created_at
- [ ] `agent_status` table — id, project_id, agent (`monitor` | `analyzer` | `planner` | `updater`), status, updated_at
- [ ] `plan_proposal` table — id, project_id, status (`pending` | `approved` | `rejected` | `applied`), changes (jsonb), created_at
- [ ] `project_plan` table — id, project_id, content (jsonb), version, finalized_at
- [ ] `plan_version` table — id, project_id, content (jsonb), created_at *(max 3 reverts enforced in code)*
- [ ] Enable Supabase Realtime on: `chat_message`, `agent_status`, `plan_proposal`, `project_plan`
